...
forvalues j=1/`=top' {
	forvalues i=1/`=currencies' {
		replace exmretlong`j' = (usdbid`i' - L.fwusdask`i') if L.rank_mom`i' == `j'+(L.mcurrused-`=top') & L.rank_mom`i' !=.
	}
}
...
forvalues j=1/`=top' {
	forvalues i=1/`=currencies' {
		replace nxmretlong`j' = (usdmid`i' - L.fwusdask`i') if L.rank_mom`i' == `j'+(L.mcurrused-`=top') & L.rank_mom`i' !=.
	}
}
...
forvalues j=1/`=top' {
	forvalues i=1/`=currencies' {
		replace exvretlong`j' = (usdbid`i' - L.fwusdask`i') if L.rank_val`i' == `j'+(L.vcurrused-`=top') &  L.vcurrused !=.
	}
}

forvalues j=1/`=bottom' {
	forvalues i=1/`=currencies' {
		replace exvretshort`j' = (L.fwusdbid`i' - usdask`i') if L.rank_val`i' == `j'	&  L.vcurrused !=.
	}
}
...
forvalues j=1/`=top' {
	forvalues i=1/`=currencies' {
		replace nxvretlong`j' = (usdmid`i' - L.fwusdask`i') if L.rank_val`i' == `j'+(L.vcurrused-`=top') &  L.vcurrused !=.
	}
}

forvalues j=1/`=bottom' {
	forvalues i=1/`=currencies' {
		replace nxvretshort`j' = (L.fwusdbid`i' - usdmid`i') if L.rank_val`i' == `j'	&  L.vcurrused !=.
	}
}
...
// First, create indicators for each currency's position across all strategies
forvalues i = 1/`=currencies' {
    // Generate net position counters (only count where strategy is active)
    gen buy_signals`i' = 0
    ...
    // Calculate net signal (-ve means net sell, +ve means net buy, 0 means conflict or no signal)
    gen net_signal`i' = buy_signals`i' - sell_signals`i'
}

// Create clean long/short indicators only where signals don't conflict
forvalues i = 1/`=currencies' {
    gen final_long`i' = (net_signal`i' > 0)
}
forvalues i = 1/`=currencies' {
    gen final_short`i' = (net_signal`i' < 0)
    // Note: Currencies with net_signal=0 will be excluded from both portfolios
}
...
// Calculate returns for positions in the combined strategy
forvalues i = 1/`=currencies' {
    // Long portfolio returns
    gen comb_long_ret`i' = 0
    // For currencies in the long portfolio
    replace comb_long_ret`i' = (usdbid`i' - L.fwusdask`i') if L.final_long`i' == 1 & final_long`i' == 0  // Exit
    replace comb_long_ret`i' = (usdmid`i' - L.fwusdask`i') if L.final_long`i' == 1 & final_long`i' == 1  // Hold
    
    // Short portfolio returns
    gen comb_short_ret`i' = 0  
    // For currencies in the short portfolio
    replace comb_short_ret`i' = (L.fwusdbid`i' - usdmid`i') if L.final_short`i' == 1 & final_short`i' == 0  // Exit
    replace comb_short_ret`i' = (L.fwusdbid`i' - usdmid`i') if L.final_short`i' == 1 & final_short`i' == 1  // Hold
...
// Calculate equally weighted returns for long and short sides separately
gen cpflong= 0 if _n>67
gen cpfshort = 0 if _n>67
...
gen cHML = cpflong + cpfshort if _n>67

// Convert back to log returns for consistency with your other calculations
replace cHML = log(1 + cHML) if _n>67
gen csumHML = sum(cHML) if _n>67
...
gen maxsumHML = sumHML if _n == 1
replace maxsumHML = max(L.maxsumHML, sumHML) if _n > 1
